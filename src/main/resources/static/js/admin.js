...
// 통계 카드 — API 연동
// 통계 카드 — API 연동 (기존 함수 수정)
function loadDashboardStats() {
    fetch('/api/admin/dashboard/stats') // 1. 바뀐 통합 주소로 요청
        .then(r => r.ok ? r.json() : null)
        .then(res => {
            if (!res) return;
            const data = res.data || res; // 2. ApiResponse 포장지 벗기기
            const el = id => document.getElementById(id);

            // 3. MySQL Mapper에서 정해준 이름표(orderCount 등)로 변경
            if (data.orderCount != null) el('dashOrderCount').textContent = data.orderCount.toLocaleString();
            if (data.totalSales != null) el('dashTotalSales').textContent = '¥' + data.totalSales.toLocaleString();
            if (data.memberCount != null) el('dashMemberCount').textContent = data.memberCount.toLocaleString();
            if (data.activeStoreCount != null) el('dashStoreCount').textContent = data.activeStoreCount.toLocaleString();

            // 전일 대비 데이터도 함께 넣어줍니다
            if (data.yesterdayOrderCount != null) {
                const diff = (data.orderCount || 0) - data.yesterdayOrderCount;
                const el2 = el('dashOrderDelta');
                el2.style.color = diff >= 0 ? '#16a34a' : '#dc2626';
                el2.textContent = (diff >= 0 ? '↑' : '↓') + ' 前日比 ' + (diff >= 0 ? '+' : '') + diff + '件';
            }
            if (data.yesterdaySales != null) {
                const diff = (data.totalSales || 0) - data.yesterdaySales;
                const el2 = el('dashSalesDelta');
                el2.style.color = diff >= 0 ? '#16a34a' : '#dc2626';
                el2.textContent = (diff >= 0 ? '↑' : '↓') + ' 前日比 ' + (diff >= 0 ? '+' : '') + '¥' + Math.abs(diff).toLocaleString();
            }
            if (data.newMemberThisWeek != null) {
                const el2 = el('dashMemberDelta');
                el2.style.color = data.newMemberThisWeek > 0 ? '#16a34a' : '#9ca3af';
                el2.textContent = data.newMemberThisWeek > 0 ? '↑ 今週 +' + data.newMemberThisWeek + '名' : '今週 新規なし';
            }
            if (data.totalStoreCount != null) el('dashStoreDelta').textContent = '全 ' + data.totalStoreCount.toLocaleString() + '店舗';
        })
...
function loadDashboardRecent() {
	// 新規会員 (ROLE_USER のみ)
	fetch('/api/admin/members?page=0&size=5&role=ROLE_USER')
	    .then(r => r.ok ? r.json() : null)
	    .then(data => {
	        const ol = document.getElementById('recentOrdersList');
	        if (!ol) return;
	        const list = (data && data.data && data.data.data) ? data.data.data : [];
	        if (!list.length) { ol.innerHTML = '<div class="list-empty">会員データがありません</div>'; return; }
	        ol.innerHTML = list.slice(0, 5).map(m => {
	            return `
	            <div class="recent-list-item">
	                <div class="recent-icon" style="background:#eff6ff;color:#2563eb;"><i class="bi bi-person"></i></div>
	                <div class="recent-info">
	                    <div class="recent-name">${m.lastName || ''} ${m.firstName || ''}</div>
	                    <div class="recent-meta">${m.email || ''} · 登録 ${(m.createdAt || '').slice(0, 10)}</div>
	                </div>
	...
	// 新規店舗
	fetch('/api/admin/stores?page=0&size=5')
		    .then(r => r.ok ? r.json() : null)
		    .then(data => {
		        const ml = document.getElementById('recentMembersList');
		        if (!ml) return;
		        const list = (data && data.data && data.data.data) ? data.data.data : [];
		        if (!list.length) { ml.innerHTML = '<div class="list-empty">店舗データがありません</div>'; return; }
		        ml.innerHTML = list.slice(0, 5).map(s => {
		            const active = !s.deletedAt;
		            return `
		            <div class="recent-list-item">
		                <div class="recent-icon" style="background:var(--primary-pale);color:var(--primary);"><i class="bi bi-shop"></i></div>
		                <div class="recent-info">
		                    <div class="recent-name">${s.name || '—'}</div>
		                    <div class="recent-meta">${catLabel[s.category] || s.category || ''} · ${s.ownerName || ''}</div>
		                </div>
		                <span class="badge ${active ? 'badge-active' : 'badge-deleted'}">${active ? '営業中' : '停止中'}</span>
		            </div>`;
		        }).join('');
		    })
		    .catch(() => {
...
function renderBarChart(wrap, data) {
    ...
    const salesMap = {};
    if (data && data.data) {
        data.data.forEach(d => { salesMap[d.saleDate] = d.totalSales || 0; });
    } else if (data && Array.isArray(data)) {
        data.forEach(d => { salesMap[d.saleDate] = d.totalSales || 0; });
    }

    const values = dateList.map(d => salesMap[d.key] || 0);
    const max = Math.max(...values, 1);
    const maxBarPx = 110;
    wrap.innerHTML = '';
    dateList.forEach((d, i) => {
        const h = Math.max(4, Math.round((values[i] / max) * maxBarPx));
        const col = document.createElement('div');
        col.className = 'bar-col';
        col.innerHTML = `
            <div class="bar-fill" style="height:${h}px;"
                 title="${d.label}: ¥${values[i].toLocaleString()}"></div>
...
function loadDonutChart() {
    fetch('/api/admin/dashboard/category-stats')
        .then(r => r.ok ? r.json() : null)
        .then(data => renderDonutChart(data && data.data ? data.data : data)).catch(() => renderDonutChart(null));
}
...
function renderDonutChart(data) {
    ...
    const total = data.reduce((s, d) => s + (d.orderCount || 0), 0); if (totalEl) totalEl.textContent = total;
    ...
    data.forEach((d, i) => {
        const pct = total ? d.orderCount / total : 0;
        const dash = pct * circ;
...
<span class="legend-pct">${total ? Math.round(d.orderCount / total * 100) : 0}%</span>        </div>`).join('');
}
...
// ── 초기 렌더 ──
if (document.getElementById('memberTbody')) renderMembers(MEMBERS);
if (document.getElementById('storeTbody')) renderStores(STORES);
if (document.getElementById('orderTbody')) renderOrders(ORDERS);
if (document.getElementById('reviewTbody')) renderReviews(REVIEWS);
// ── 모달 ──
